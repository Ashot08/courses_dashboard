...
class Key {
    ...
    private function insertKey($key){
        global $wpdb;
    ...
    private function connectKeyWithDirector(int $key_id, int $director_id){
        global $wpdb;
        $table_name = $wpdb->prefix . "c_dash__directors_keys";
        $wpdb->insert( $table_name, [ 'key_id' => $key_id, 'director_id' =>  $director_id ]);
        return $wpdb->insert_id;
    }
    ...
    private function connectKeyWithProgram(int $key_id, int $program_id){
        global $wpdb;
        $table_name = $wpdb->prefix . "c_dash__programs_keys";
        $wpdb->insert( $table_name, [ 'key_id' => $key_id, 'program_id' =>  $program_id ]);
        return $wpdb->insert_id;
    }

    public function createAndAttachKey($director_id, $program_id){
        $key_id = $this->createKey();
        $this->connectKeyWithDirector($key_id, $director_id);
        return $this->connectKeyWithProgram($key_id, $program_id);
    }
